...
function updateKDChart() {
  ...
  if (kdChart) {
    kdChart.data.labels = [...Array(TOTAL_DAYS).keys()];
    ...
    kdChart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: {
            ticks: { color: "#bdbdbd", callback: (val, idx) => dates[idx] || "" },
...
function updateMAChart() {
  ...
  // ✅ Add PRICE line (same axis, very visible)
  datasets.push({
    label: "PRICE",
    data: priceSeries,
    order: 1000,
  ...
  if (maChart) {
    maChart.data.labels = [...Array(TOTAL_DAYS).keys()];
    ...
    maChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: "top",
            labels: { color: "#eeeeee", usePointStyle: true }
          },
          annotation: {
            drawTime: "beforeDatasetsDraw",
            annotations: shadedBoxes
          },
          zoom: {
            pan: { enabled: true, mode: "x", modifierKey: "ctrl" },
            zoom: { wheel: { enabled: true }, pinch: { enabled: true }, mode: "x" }
        ...
        scales: {
          x: {
            ticks: { color: "#bdbdbd", callback: (v,i)=>dates[i] || "" },
            grid: { color: "rgba(255,255,255,0.06)" }
          },
          y: {
            ticks: { color: "#bdbdbd" },
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: {
            color: "#bdbdbd",
            callback: function (val, idx) { return dates[idx] || ""; }
          },
          grid: { color: "rgba(255,255,255,0.06)" }
        },
        y: {
...
function runSimulator() {
  ...
  for (let i = from; i <= to; i++) {
    ...
    if (shares > 0 && sellIdxSet.has(i)) {
      ...
      if (fi !== -1 && Number.isFinite(priceSeries[fi])) {
        ...
        cash += shares * sellPx;
        rows.push({ side: "SELL", date: dates[fi], price: sellPx, shares, cash });
        shares = 0;
        trades++;
      }
      continue;
    ...
    if (shares === 0 && buyIdxSet.has(i)) {
      ...
      if (fi !== -1 && Number.isFinite(priceSeries[fi])) {
        ...
        const canBuy = Math.floor(cash / buyPx);
        if (canBuy > 0) {
          shares = canBuy;
          cash -= shares * buyPx;
          rows.push({ side: "BUY", date: dates[fi], price: buyPx, shares, cash });
          trades++;
        }
      }
    }
  ...
  if (meta) {
    meta.textContent =
      ...
      `Fill: ${fillMode} | Fund: ${fund.toFixed(2)} | ` +
      `Conditions: ${[
